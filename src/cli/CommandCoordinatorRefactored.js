...
export class CommandCoordinator {
  ...
  /**
   * Start the learning system to monitor and improve
   */
  async startLearning() {
    try {
      await this.learningController.startLearning({
        mode: 'continuous',
        collectMetrics: true,
        adaptPatterns: true
      });
  ...
  async logCommandExecution(command, options, phase, metadata = {}) {
    try {
      if (this.learningController) {
        await this.learningController.recordCommandExecution({
          command,
  ...
  async getLearningMetrics() {
    try {
      if (this.learningController) {
        return await this.learningController.getMetrics();
      }
  ...
  async stopLearning() {
    try {
      if (this.learningController) {
        await this.learningController.stopLearning();
        if (this.globalConfig.verbose) {
